...
static uint32_t iec61850_hash (gconstpointer v);
static proto_item *get_iec61850_item(tvbuff_t *tvb, proto_tree *parent_tree, const int32_t proto_iec61850);
static void store_invoke_data(packet_info *pinfo, uint32_t invokeID, iec61850_value_req * data);
static iec61850_value_req *load_invoke_data(packet_info *pinfo, uint32_t invokeID);
/*static void free_invoke_data(packet_info *pinfo, uint32_t invokeID);*/
    ...
    if(fi->hfinfo != NULL && fi->hfinfo->name != NULL)
    {
        ...
        switch(fi->hfinfo->type)
        {
            ...
            case FT_BOOLEAN:
                ...
                if(fvalue_get_uinteger(fi->value)) /* if value is non-zero, set bool to bitmask */
                    boolean = bitmask;
            ...
            case FT_UINT32:
                ...
                {
                    pdata->listOfAccessResult = fvalue_get_uinteger(fi->value);
                }
                if(g_str_equal(fi->hfinfo->name,"listOfVariable"))
                {
                    pdata->listOfVariable = fvalue_get_uinteger(fi->value);
                    if(pdata->listOfVariable > 0)
                    {
                        pdata->listOfVariable_array = wmem_alloc0(pdata->actx->pinfo->pool, sizeof(uint8_t * )*pdata->listOfVariable);
                        pdata->elements = -1;
                    }
                }
                ...
                if(g_str_equal(pdata->request,"CmdTerm")) /* CMDTERM*/
                {
                    ...
                    { /* check name variable to determine if we have a CmdTerm, Addcause or Both */
                        ...
                        pdata->listOfVariable_array[pdata->elements] = fvalue_get_uinteger(fi->value);
                    }
                    ...
                    if(pdata->listOfAccessResult && 
                        ...
                        pdata->itemIndex == -1)/* start condition */
                    {
                        pdata->structureSize = fvalue_get_uinteger(fi->value);
                        break;
                    }
                    if(pdata->listOfAccessResult && 
                ...
                {
                    item = proto_tree_add_uint(tree, fi->hfinfo->id,tvb, offset, fi->length,  fvalue_get_uinteger(fi->value)); 
                }
                break;
            case FT_CHAR:
            case FT_INT8:
            ...
            case FT_INT32:
                ...
                item = proto_tree_add_int(tree, fi->hfinfo->id,tvb, offset, fi->length,  fvalue_get_sinteger(fi->value)); 
                break;
            case FT_STRING:
                item = proto_tree_add_string(tree, fi->hfinfo->id,tvb, offset, fi->length,  fvalue_get_string(fi->value));
            ...
            case FT_BYTES:
            {
                if(g_str_equal(fi->hfinfo->name, "bit-string"))
                {
                    ...
                    if(g_str_equal(pdata->request,"RPT") && pdata->listOfAccessResult)
                    {
                        ...
                        count = iec61850_print_bytes(strbuf,fvalue_get_bytes_data(fi->value),fi->length, padding);
                ...
                {
                    proto_tree_add_bytes_format_value(tree, fi->hfinfo->id,tvb, offset, fvalue_get_bytes_size(fi->value), fvalue_get_bytes_data(fi->value), "%f",tvb_get_ieee_float(tvb, 1, ENC_BIG_ENDIAN) );
                    break;
                }

                item = proto_tree_add_bytes(tree, fi->hfinfo->id,tvb, offset, fvalue_get_bytes_size(fi->value), fvalue_get_bytes_data(fi->value));
                if(g_str_equal(fi->hfinfo->name, "octet-string"))
                {
                    if(g_str_equal(pdata->request,"CmdTerm") && pdata->listOfAccessResult)
                    {
                        ...
                        {
                            uint8_t * ostr = wmem_alloc0(pdata->actx->pinfo->pool,fvalue_get_bytes_size(fi->value)+1);/* make zero terminated string */ 
                            memcpy(ostr, fvalue_get_bytes_data(fi->value), fvalue_get_bytes_size(fi->value));
...
static int32_t dissect_iec61850_mmstree(proto_tree *subtree, tvbuff_t *tvb, asn1_ctx_t *actx, uint8_t * request)
{
    tree_data data;
    if(g_mms_tree == NULL)
    {
...
static int32_t GetNameList(tvbuff_t *tvb, int32_t offset, proto_item *item, asn1_ctx_t *actx, int32_t res, iec61850_value_req *val)
{
    ...
    if(res == 0) /* request */
    {
        ...
        ws_assert(val);
        request = (uint8_t *)wmem_strbuf_get_str(val->data);
    ...
    if(val != NULL)
    {
        dissect_iec61850_mmstree(subtree, tvb, actx, (uint8_t *)wmem_strbuf_get_str(val->data));
    }
    else
    {
        dissect_iec61850_mmstree(subtree, tvb, actx, "");
...
{ 
    ...
    uint8_t * serviceName = "GetDataValue";
    uint8_t * request = "";
...
static int32_t SetDataValue(tvbuff_t *tvb, int32_t offset, proto_item *item, asn1_ctx_t *actx, int32_t res, iec61850_value_req * val)
{
    ...
    proto_tree *subtree=NULL;
    iec61850_private_data_t *private_data = (iec61850_private_data_t*)iec61850_get_private_data(actx);
    uint8_t * fieldName = "SetDataValue";
    uint8_t * request = "";
    ...
    if(tvb_reported_length_remaining(tvb, offset) > 0) /* while for multiple PDU in 1 packet (may be possible?...) */
    {
        ...
        switch(private_data->MMSpdu)
        {
            case 0:/* confirmed-req */
                ...
                g_strstrip((uint8_t *)wmem_strbuf_get_str(sessiondata.data));
...
static uint32_t iec61850_hash (gconstpointer v)
{
    ...
    uint64_t conv = (uint64_t)key->conversation;
    uint64_t invokeID = (uint64_t)key->invokeID;
